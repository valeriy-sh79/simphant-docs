...
This repository contains a bilingual Docusaurus documentation site for SimPhant.

## Content Sources

- English source files are kept in `Word_EN`.
- Russian source files are kept in `Word_RU`.
- Ukrainian source files are kept in `Word_UA`.
...
- The current `url` value in `docusaurus.config.js` is a placeholder and should be updated when you know the final deployment domain (Tell me your real domain or hosting target and I can finish the deployment configuration).
