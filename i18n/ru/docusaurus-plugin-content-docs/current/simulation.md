...
Для решения проблемы избыточных связей применяется метод регуляризации матрицы (регуляризация Тихонова). Принцип его работы таков: вместо обеспечения бесконечной жесткости связей в диагональ матрицы вносится ничтожно малая величина «податливости» упругости. Это позволяет мгновенно устранить вырожденность матрицы без изменения ее размерности.

В контексте регуляризации Тихонова значение, задаваемое в пользовательском интерфейсе (например, -1E-7), физически представляет собой податливость связи (м/Н) — величину, обратную жесткости. С математической точки зрения это эквивалентно установке в сочленении виртуальной пружины, что позволяет распределить нагрузку между сочленениями.
